...
def load_app_env() -> None:
    """Load env files; ai_architecture/.env overrides frontend/.env.local."""
    load_dotenv(DEFAULT_INDEX_ROOT / "frontend" / ".env.local")
    load_dotenv(DEFAULT_INDEX_ROOT / ".env", override=True)


def get_api_key() -> str:
    """Prefer API keys from ai_architecture/.env over frontend/.env.local."""
    global _API_KEY
    ...
    if not api_key:
        raise RuntimeError(
            "Set AI_BUILDER_TOKEN or SUPER_MIND_API_KEY in ai_architecture/.env "
